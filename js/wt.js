...
app.controller('ctrl',['$scope','packages','$timeout', function($scope,packages,$timeout){
	...
	return{
		...
		controller: function($scope,$timeout,packages){
			...
			$scope.showPkg = function(pkgCategory, pkgName){
				...
				$timeout(function(){
					$scope.pkgName = pkgName;
					$scope.pkgCategory = pkgCategory ;
					$scope.pkgDetail = $scope.pkg[pkgCategory][pkgName];
				},400);
				$timeout(function(){$('#pkgPane').removeClass('rotate');},400);
			}	

			$scope.setPhotoPkg = function(){
				
				if($scope.activePkg!=1){
					$scope.pkg = packages.photo;
					$scope.activateButton(1,2);
					$scope.showPkg("Hourly","Royalty")
					$scope.activePkg = 1;
				}	
			}

			$scope.setVideoPkg = function(){
				
				if($scope.activePkg!=2){
					$scope.pkg = packages.video;
					$scope.activateButton(2,1);
					$scope.showPkg("Hourly","Royalty")
					$scope.activePkg = 2
				}	
			}
